...
BOOL CThread::Start(unsigned int core_id)
{
        				...
        				(void *)this))
	{
		...
		if (core_id != NO_SPEC_CORE)
		{
			...
			{
				EL_LOG_ERROR("Could not set thread %lu's CPU Affinity to core %d, continuing...",
						(unsigned long)m_handle, core_id);
			}
			else
			{
				EL_LOG_DEBUG("set thread %lu's CPU Affinity to core %d.",
						(unsigned long)m_handle, core_id);
			}
		}
	}
...
void volatile CThread::Join(int timeout)
{
    ...
    while(m_bRun)
    {
    	...
    	{
    		EL_LOG_ERROR("wait thead exit failed, kill -9 directly.");
...
{
	EL_LOG_DEBUG("receive signo %d.", signo);
	pthread_exit(0);
}
